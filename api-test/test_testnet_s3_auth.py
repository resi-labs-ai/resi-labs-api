...
def check_testnet_balance(wallet: bt.wallet) -> bool:
    ...
    try:
        ...
        if balance.tao < 1.0:
            ...
            print_info("Get free testnet TAO:")
            print_info("  - btcli wallet faucet --wallet.name YOUR_WALLET --subtensor.network test")
            print_info("  - https://app.minersunion.ai/testnet-faucet")
            return False
        else:
...
def check_validator_status(wallet_name: str, hotkey_name: str) -> bool:
    ...
    print_info("This will check if your hotkey is registered as a validator on testnet")
    ...
    try:
        ...
        if hotkey_address in metagraph.hotkeys:
            ...
            print_success(f"Hotkey is registered on testnet subnet {TESTNET_SUBNET}!")
            print(f"   Position: {idx}")
            print(f"   Stake: {stake:.4f} testnet TAO")
            ...
            if is_validator:
                ...
                print_info("  1. Ensure sufficient stake (typically 100+ testnet TAO)")
                print_info("  2. Run: btcli subnet set_weights --subtensor.network test --netuid 428")
                print_info("  3. Wait for validator permit to be granted")
            
            return is_validator
        else:
            print_error("Hotkey is NOT registered on testnet subnet 428")
            print_info("Register first: btcli subnet register --subtensor.network test --netuid 428")
            return False
